...
def authorize_token(request: Request):
    ...
    try:
        ...
        pass
    request_username_header: Tuple[bytes] = (
        "x-request-user".encode(),
        request_username.encode(),
    )
    request_role_header: Tuple[bytes] = (
        "x-request-role".encode(),
        request_role.encode(),
    )
...
def check_api_permission(path, request_role=None):
    ...
    if accepted_role == ALLOW_ALL or request_role in accepted_role:
        return
    raise PermissionDeniedException("You are not allowed to access this API")
